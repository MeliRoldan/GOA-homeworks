...
arr_of_user_names = ["Meli", "Nika", "Giorgi", "Luka", "Ana"] # მონაწილეების სახელების სია
arr_of_operator = ["+", "-", "*", "/", ">", "<"] # მათეტიკური და შედარების ოპერატორების სია
...
while arr_of_user_names: # while ციკლი რომელი ამეორებს შიგნით მოცემულ მოქმედებებს სანამ მონაწილეების სია ცარიელი არაა
    ran_participant = random.choice(arr_of_user_names) # რანდომ სახელის ცვლადი
    ...
    while i < 5 : # while ციკლი რომელიც მეორდება 5-ჯერ
        ...
        ran_operator = random.choice(arr_of_operator) # რენდომ ოპერატორი
    ...
    arr_of_results[ran_participant] = points # შედეგების ლექსიკონში შენახულ მონაწილიის სახელს, რომელიც რენდომ მონაწილეს უდრის ვანიჭებთ საბოლოო ქულების მნიშვნელობას
    arr_of_user_names.remove(ran_participant) # მონაწილეების სახელების სიისან ვშლით რენდომად ამორჩეულ მონაწილეს რათა აღარ განმეორდეს
...
for participant, result in arr_of_results.items(): # for ციკლი შედეგებისს ლექსიკონის თითოეულ გასაღებს და მისი მინიშვნელობას არქმევს participant-ს და result-ს, და გამოაქვს სათითაოდ სტრინგად
    print(f"{participant}: {result}.") # f მეთოდი ახდენს სტრიგნის ფორმატირებას, ხოლო {}-თი შეგვიძლია ცვლადების, ფუნქციების ფორმატირება.